...
- **Hard to reverse** - a schema, a wire format, a public interface, anything an issue blocked on this one builds on: stop. `add` a `grilling` child of this impl issue, `block` the issue on it, and release the claim. The human answers it as a normal round.
