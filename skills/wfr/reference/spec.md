...
- **Another program uses it** - a library, a CLI in a pipeline, a wire format, a tool like this one. "As a caller I want `frontier` to list takeable tickets" only restates the signature. Enumerate the **entry points**: each one, what it accepts, returns, refuses.
