...
* Fog goes stale the moment the ticket that lit it resolves, so read the body back before you call it charted.
...
**Never `block` a ticket on the spec itself** - parentage already records that it came from there. Instead, `block` each ticket on the sibling tickets it actually depends on, mirroring the dependency shape the map's decisions established. A spec-shaped block puts the whole frontier behind one ticket that only closes once every ticket is already done.
...
Read [deliverable](reference/deliverable.md) when you take an impl ticket off the frontier.
