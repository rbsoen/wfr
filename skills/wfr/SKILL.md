...
**A prototype also goes in the store, but it has to run.** Text goes straight to the store where executable code cannot: it needs a real path under the project tree while being worked on. So it round-trips:

```
wfr.py cat FILE shell.html > <project>/proto/shell.html   # export FILE DIR for all of it
<run it, render it, iterate>
wfr.py put FILE shell.html --issue N < <project>/proto/shell.html
<delete the disk copy>
```
